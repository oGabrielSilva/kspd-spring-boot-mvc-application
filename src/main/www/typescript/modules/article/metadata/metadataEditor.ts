...
export function metadataEditor(slug: string) {
  ...
  metadataUserTable.tBodies[0]
    ...
    .forEach((button) => (button.onclick = () => onButtonRemoveUserClicked(button)));
}

function generateUserTableRow(user: User) {
  return generateHTML({
    htmlType: 'tr',
    attributes: [{ key: 'title', value: user.email }],
    children: [
      {
        htmlType: 'td',
        value: user.email,
        ...
        children: [
          {
            ...
            onClick: (btn) => onButtonRemoveUserClicked(btn as HTMLButtonElement),
          },
        ],
      },
    ],
...
function onButtonRemoveUserClicked(button: HTMLButtonElement) {
  console.log(button.dataset.hash);
}
